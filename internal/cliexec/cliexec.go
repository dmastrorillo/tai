...
//
// It is the single seam used by both cmd/tai (the production binary) and
// internal/cmdtest (the test harness) so panic-recovery behaviour cannot
// drift between the two. A panic during command execution surfaces as an
// errcode.InternalError, written to stderr by the caller via cliout.
package cliexec
