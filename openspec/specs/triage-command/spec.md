...
The `triage-command` capability defines the bundled `/tai-triage:triage` slash command: its invocation forms, the four-phase loop (scope resolution, investigation, decision loop, recap), the presentation contract that puts an investigation rather than a stored record in front of every decision, the dismissal-debate contract, the batch-override convention, the frontmatter the plugin host reads when placing the command, and the rule that all state changes route through the `tai triage` CLI verbs rather than direct database writes. The CLI verbs introduced by `triage` (`tai triage list`, `tai triage show`, `tai triage accept`, `tai triage dismiss`, `tai triage complete`, `tai triage status`, `tai triage forget`) are the keyboard; this slash command is the pianist — everything subjective (severity weighting in presentation order, the sparring debate when a user dismisses, cognitive-bias checks, per-batch override semantics in conversation) lives here, not in the CLI.
