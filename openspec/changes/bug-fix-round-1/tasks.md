...
- [x] 7.1 Added `core/internal/notices/` exposing the `BeforeCommand` / `AfterCommand` pair that brackets the foreground command, reading and writing `<dataDir>/state/first-run.json` with shape `{"first-run": "<ISO-8601 UTC>"}`. Best-effort write — does not error the caller on failure. Named for what it owns (both unsolicited stderr notices, and the rule that they never stack) rather than `firstrun`, because the banner-vs-hint decision has to be made in one place.
- [x] 7.2 In `core/cmd/tai/main.go`, after the foreground command's Run returns (regardless of error), and BEFORE the existing brief `Waiter.Wait()` for the background poll, call `firstrun.MaybeEmit(os.Stderr, dataDir)`. Suppress the call when `len(os.Args) == 1` AND stderr is not a TTY (per the spec's CI-friendliness rule).
- [x] 7.3 In `core/internal/sync/banner.go`, when the first-run hint has fired this invocation, set the banner's `last-banner-date` to today so the daily banner is eligible to fire NEXT day instead of stacking.
- [x] 7.4 The hint text lives in `core/internal/plugins` as `PostInstallHint` / `AggregateInstallHint` so both call sites share the wording, and is printed by the verbs in `core/internal/cmd/plugins.go` after a successful install/update. Printing at the verb rather than inside `Install`/`Update` means the auto-install loop gets no per-plugin hint by construction, and the error paths suppress it by returning first — neither needs a writer to be threaded through and muted.
- [x] 7.5 In `core/internal/sync/plugins.go` (or wherever the auto-install loop lives), accumulate the installed plugin names and emit the aggregate hint `→ <N> plugin(s) installed — run \`tai <name> help\` for any of: <comma-list>.` after the loop completes.
- [x] 7.6 e2e tests: first-run hint emission on a clean data dir; suppression on a populated marker; pending-update banner skipped on first run with `last-banner-date` advanced; post-install hint after `tai plugins install triage`; suppression after a failing install; aggregate hint after a multi-plugin auto-install.

## 8. Bug 8 — third-party install prompt + trust cache

- [x] 8.1 In `core/internal/plugins/`, add a helper `IsBuiltin(src Source) bool` that returns true when the source matches an entry in `registry.go::builtin` (host/repo/subpath equal). Used by the install/update path.
- [x] 8.2 In `core/internal/plugins/install.go` (entry: `Install`) and `update.go`, before the fetch step, branch on `IsBuiltin(src)`. When false: print the third-party prompt to `stderr`, read confirmation from `stdin`. Honour `opts.AssumeYes` (new field, threaded from a new `--yes`/`-y` flag on `tai plugins install` and `tai plugins update`). Non-TTY without `AssumeYes` fails with `PluginThirdpartyUnconfirmed`. TTY-detection helper lives in `pkg/cliout` (`IsTTY`).
- [x] 8.3 In `core/internal/cmd/plugins.go`, wire the `--yes`/`-y` boolean flag on `tai plugins install` and `tai plugins update`. Plumb the value into `InstallOptions.AssumeYes`.
- [x] 8.4 Add a `core/internal/plugins/trust.go` (or similar) with `TrustStore` reading/writing `<dataDir>/state/trust.json`. Shape: `{"trust":[{"repo-url":"...","plugins-yml-sha256":"..."}]}`. Provide `Load`, `Save`, `Get(repoURL) (hash, ok)`, `Put(repoURL, hash)`.
- [x] 8.5 In `core/internal/sync/trust.go`, `confirmThirdPartyPlugins` is called from `autoInstallPluginsFromYAML` immediately after the `plugins.yml` parse — unexported and taking `Options`, since every caller is inside the sync package and the option struct already carries the streams and the flag. The helper: short-circuits if no third-party entries; computes sha256 of the verbatim file bytes; loads the trust store; compares against stored hash; either proceeds silently, prompts, or fails per the spec.
- [x] 8.6 Wire the new `--trust-third-party` flag on `tai sync`. Plumb into the `ConfirmThirdPartyPlugins` helper as the non-TTY/non-interactive bypass.
- [x] 8.7 e2e tests: confirm three paths — TTY yes path persists hash; TTY no path aborts with `PLUGIN_THIRDPARTY_UNCONFIRMED`; non-TTY without `--trust-third-party` aborts; non-TTY with the flag proceeds and persists. Plus: internal-only plugins.yml never prompts and never writes trust.json.
