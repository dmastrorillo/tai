...
This change adds a **board**: a browser surface, served on loopback by the triage plugin, that shows every pending comment in a scope at once — with the same content `tai show` renders today — and lets the developer accept, dismiss, or leave each one alone in a single pass, attaching a free-text note to any of them. The board records those calls as **intents** and writes them to a file. It never touches the database.
...
- **New capability `triage-board`**, adding three verbs to the triage plugin:
  - `tai triage board [--pr N | --branch NAME]` — binds an HTTP listener on `127.0.0.1:0`, serves the board under an unguessable random path, opens the developer's browser (best-effort), and blocks. On submit it writes the intents artifact and exits `0`.
  - `tai triage board intents [--pr N | --branch NAME]` — reads the intents artifact for a scope and emits it as markdown for the AI to consume.
  - `tai triage board status [--pr N | --branch NAME]` — reports whether an intents artifact exists for the scope and when it was written. Exists so an agent harness without process-exit notification can poll.
- **New on-disk artifact**: the intents file, at `<TAI_DATA_DIR>/plugins/triage/state/intents/<owner>--<name>--<scope>.json`. Written only by `tai triage board`, read only by `tai triage board intents` and `tai triage board status`.
...
- **Two new error codes**: `TRIAGE_BOARD_UNAVAILABLE`, `TRIAGE_NO_INTENTS`.

## Non-goals

- **The board does not write to the database.** Comment status is changed only by `tai accept` / `tai dismiss` / `tai complete`, invoked by the AI after the triage conversation. This is the property that makes the change additive; it is a constraint, not an implementation detail that may be relaxed later for convenience.
- **No change to the storage schema.** No migration, no new status value, no change to the comment state machine. Intents are not a comment state.
...
- **The board does not re-import, and does not run the investigation.** It renders what is already `pending` in the database at the moment it starts.

## Capabilities

### New Capabilities

- `triage-board`: the board's three verbs, the loopback server's lifecycle and security posture, what the board renders, the intents artifact's format and location, and the two new error codes.
...
- **Assets**: `plugins/triage/assets/commands/triage.md` gains the board-offer step, the intent-equivalence rule, and the batch-confirmation carve-out.
- **Tests**: new `TC-BRD-*` category in `plugins/triage/test-cases.md`.
- **Docs**: `CONTEXT.md` already carries the `Board`, `Intent`, `Decision`, and `Bulk pass` glossary entries.
