...
This change adds a **board**: a browser surface, served on loopback by the triage plugin, that shows every pending comment in a scope at once — with the same content `tai triage show` renders today — and lets the developer accept, dismiss, or leave each one alone in a single pass, attaching a free-text note to any of them. The board records those calls as **intents** and writes them to a file. It never touches the database.
...
- `triage-board`: the board's two verbs, the loopback server's lifecycle and security posture, what the board renders, the intents artifact's format and location, and the two new error codes.
