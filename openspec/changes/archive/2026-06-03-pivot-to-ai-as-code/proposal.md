...
- **GitHub release pipeline changes** (DEFERRED): a release of the tai repo will eventually need to produce multiple binaries (core `tai` + each first-party plugin) across the OS/arch matrix. The plugin-host spec pins the asset-name convention (`tai-plugin-<name>-<os>-<arch>`, with `.exe` suffix on Windows), but the CI wiring itself is postponed to a follow-up proposal so the choice between a hand-rolled workflow and GoReleaser can be made deliberately. Until that proposal lands, releases are built and attached manually.
