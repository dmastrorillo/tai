...
- **`triage-command` is resynced to the shipped command.** The capability spec had drifted behind its own asset: it named the command `/tai:triage`, addressed the plugin's verbs bare (`tai accept`), described placement by a `tai install` that no longer exists along with a `content_hash` and ledger the plugin stopped shipping, asked for three decision verbs, and described step 1 as surfacing `tai triage show` verbatim — which TC-AST-003 now forbids outright. Every requirement in the capability is restated against the shipped `triage.md`, and the capability's Purpose paragraph is corrected in place since archiving merges requirements and leaves prose untouched. No behaviour changes; the spec catches up to four commits that changed the asset and its test cases without it.
